...
从形态上来说，可以分成索引下推，覆盖索引，前缀索引，全文索引，联合索引，唯一索引和主键索引。（下面的定义，可以直接一股脑说出来，也可以等面试官问）

0. 索引下推优化，可以在索引遍历过程中，对索引中包含的字段先做判断，直接过滤掉不满足条件的记录，减少回表次数；
...
1. 和二叉树，如平衡二叉树，红黑树比起来，B+树是多叉树，比如MySQL默认是1200叉树(MySQL页大小是16K，主键用bigint类型8字节 指针6字节 16000/(8+6)=1142 ≈ 1200)，同样数据量，高度要比二叉树低；
...
https://blog.csdn.net/u011783999/article/details/105334301
...
3. 字符串没有引号导致类型转换，索引隐式转换
3. 联合索引非最左匹配原则
3. where语句中使用的or其中一个条件需要全表扫描
...
分析：后端圈开车的多年老司机经常说的就是，"MySQL 单表最好不要超过 2000W”,“单表超过 2000W 就要考虑数据迁移了”,“你这个表数据都马上要到 2000W 了，难怪查询速度慢”
...
![image-20230615124804088](/Users/11126518/knowledge/interview_skills_BAT/sql/img/index_page.png)

如上图计算得：**Total =x^(z-1) \*y 也就是说总数会等于 x 的 z-1 次方 与 Y 的乘积**。
...
所以如果叶子结点的数据一行小于1k，假设占用500B，那么就可以存接近**5000w**的数据了。
...
很多人会觉得一定要count主键这样才是对的，而且更快

答案：

> Count(字段)

**统计符合查询条件的记录中，函数指定的参数不为 NULL 的记录有多少个**。分为字段是是否有索引，索引还分是聚簇索引还是非聚簇索引都有关系。因为相同数量的二级索引记录可以比聚簇索引记录占用更少的存储空间，所以二级索引树比聚簇索引树小，这样遍历二级索引的 I/O 成本比遍历聚簇索引的 I/O 成本小，因此「优化器」优先选择的是二级索引
